"""A policy used in training meta reinforcement learning algorithms, trains the encoder via Contrastive Learning.
...
class GeoContextConditionedPolicy(nn.Module):
    """A policy that outputs actions based on observation and known context.
    ...
    def compute_contrastive_loss(self, 
        ...
        pos_sim = torch.cosine_similarity(emb_current_traj, emb_positive_traj)
        neg_sim = torch.cosine_similarity(emb_current_traj.unsqueeze(1), emb_negative_traj, dim=-1)
        
        contrastive_loss = -torch.mean(torch.log(torch.exp(pos_sim) / (torch.exp(pos_sim) + torch.sum(torch.exp(neg_sim), dim=1))))
